...
airbnb_data_sub <- airbnb_train %>% 
  select(-c(signup_app, signup_flow, affiliate_channel, first_affiliate_tracked))
...
```{r}
randomforest_model <- function(training, holdout, samplesize) {
  # Tunning mtry
...
```{r}
rf_under_full_result <- lapply(as.list(1:5), function(t){
...
```{r}
names(rf_under_full_result) <- lapply(as.list(1:5), function(t){paste0("fold", as.character(t))}) %>% unlist()
sapply(rf_under_full_result, "[", "mis_class")
```

```{r}
rf_sub_result <- lapply(as.list(1:5), function(t){
...
```
```{r}
names(rf_sub_result) <- lapply(as.list(1:5), function(t){paste0("fold", as.character(t))}) %>% unlist()
sapply(rf_sub_result, "[", "mis_class")
```
...
```{r}
save(rf_full_result, rf_sub_result, rf_under_full_result, rf_misclass,
