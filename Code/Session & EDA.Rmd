...
sessions <- sessions %>% filter(user_id %in% c(users_train$id, users_test$id))
# Only left with 7801002 obs, originally have 10567737 obs, which is 2766735 obs less
```
...
# Compute the number of actions each user performed
actions <- sessions %>% select(-c(device_type, secs_elapsed)) %>% 
  distinct() %>% group_by(user_id) %>% summarise(total_action = n())
...
```{r message=FALSE}
devices %>% filter(Apple != 0) %>% ggplot(aes(x = Apple)) + geom_histogram()
# devices %>% filter(Apple != 0) %>% ggplot(aes(x = log(Apple))) + geom_histogram()
devices %>% filter(Apple != 0) %>% ggplot(aes(x = (Apple^(1/3)))) + geom_histogram()

devices %>% filter(Android != 0) %>% ggplot(aes(x = Android)) + geom_histogram()
# devices %>% filter(Android != 0) %>% ggplot(aes(x = log(Android))) + geom_histogram()
devices %>% filter(Android != 0) %>% ggplot(aes(x = (Android^(1/3)))) + geom_histogram()

devices %>% filter(Windows != 0) %>% ggplot(aes(x = Windows)) + geom_histogram()
# devices %>% filter(Windows != 0) %>% ggplot(aes(x = log(Windows))) + geom_histogram()
devices %>% filter(Windows != 0) %>% ggplot(aes(x = (Windows^(1/3)))) + geom_histogram()

devices %>% filter(Other != 0) %>% ggplot(aes(x = Other)) + geom_histogram()
devices %>% filter(Other != 0) %>% ggplot(aes(x = log(Other))) + geom_histogram() # bimodal distribution -> binning
...
write_csv(new_session, paste0(data_path, "cleaned_session.csv"))
```
