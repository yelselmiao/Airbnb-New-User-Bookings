...
airbnb_train <- airbnb_train %>% mutate(
...
airbnb_train %>% ggplot(aes(x = date_account_created)) + geom_histogram()
airbnb_train %>% ggplot(aes(x = date_first_active)) + geom_histogram()
```
...
library(caret)
folds <- createFolds((airbnb_train %>% lapply(as.factor))$country_destination, k = 5)
```


### Save Data
```{r}
# save(airbnb_train, folds, file = paste0(here::here(),"/Data/","airbnb.RData"))
```
